...
use editor::Editor;
use gpui::{
    list, prelude::*, px, AnyElement, App, Context, Entity, EventEmitter, FocusHandle, Focusable,
    ListState, Render, SharedString, WeakEntity, Window,
};
...
use ui::ThreadItem;
use ui::{prelude::*, utils::platform_title_bar_height, Tooltip, WithScrollbar};
...
impl ThreadsArchiveView {
    ...
    fn update_items(&mut self, cx: &mut Context<Self>) {
        let sessions: Vec<_> = ThreadMetadataStore::global(cx)
            .read(cx)
            .archived_entries()
            .sorted_by_cached_key(|t| t.created_at.or(Some(t.updated_at)))
            .rev()
            .cloned()
            .collect();
    ...
    fn render_list_entry(
        ...
        match item {
            ...
            ArchiveListItem::Entry {
                ...
                let deletes_underlying_thread = thread.agent_id.as_ref() == agent::ZED_AGENT_ID.as_ref();
                ...
                let thread_item = ThreadItem::new(id, thread.title.clone())
                    .icon(icon)
                    .timestamp(timestamp)
                    ...
                    .action_slot(
                        IconButton::new("remove-archive-entry", IconName::Close)
                            .style(ButtonStyle::Filled)
                            .icon_size(IconSize::Small)
                            .icon_color(Color::Muted)
                            .tooltip({
                                let label = if deletes_underlying_thread {
                                    "Delete Thread"
                                } else {
    ...
    fn delete_thread(
        ...
        if agent.as_ref() == agent::ZED_AGENT_ID.as_ref() {
            let delete_task =
                ThreadStore::global(cx).update(cx, |store, cx| store.delete_thread(session_id, cx));
            delete_task.detach_and_log_err(cx);
        }
    }
